...
local function pop(pool)
	assert(#pool > 0, "map.lua: pool empty — increase map size")
	local t = pool[#pool]
	pool[#pool] = nil
	return t.x, t.y
end
...
function M.generate(width, height)
	...
	local roomCount = love.math.random(MC.roomCountMin, MC.roomCountMax)
	for i = 1, roomCount do
		local rx   = love.math.random(3, width  - 3)
		local ry   = love.math.random(3, height - 3)
		local size = love.math.random(MC.roomSizeMin, MC.roomSizeMax)
		carveRoom(map, rx, ry, size, width, height, log)
	end
	...
	for i = 1, MC.shopCount do
		...
		map[sy][sx] = T.shop
		log[#log+1] = {x=sx, y=sy, v=T.shop}
	end

	local enemyCount = math.min(love.math.random(MC.enemyCountMin, MC.enemyCountMax), #pool)
	for i = 1, enemyCount do
		local x, y = pop(pool)
		map[y][x] = T.enemy
		log[#log+1] = {x=x, y=y, v=T.enemy}
	end

	local goldCount = math.min(love.math.random(MC.goldCountMin, MC.goldCountMax), #pool)
