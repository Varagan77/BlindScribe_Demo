...
function M.generate(width, height)
	...
	local roomCount = love.math.random(Cfg.ROOM_MIN, Cfg.ROOM_MAX)
	for i = 1, roomCount do
		local rx   = love.math.random(3, width  - 3)
		local ry   = love.math.random(3, height - 3)
		local size = love.math.random(Cfg.ROOM_SIZE_MIN, Cfg.ROOM_SIZE_MAX)
	...
	for i = 1, Cfg.SHOP_COUNT do
		local sx, sy = pop(pool)
		map[sy][sx] = 2
		log[#log + 1] = { x = sx, y = sy, v = 2 }
	end

	local enemyCount = math.min(love.math.random(Cfg.ENEMY_MIN, Cfg.ENEMY_MAX), #pool)
	...
	local goldCount = math.min(love.math.random(Cfg.GOLD_MIN, Cfg.GOLD_MAX), #pool)
