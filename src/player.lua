...
function player_draw()
	...
	local ts = Cfg.TILE_SIZE
	local cx = player.grid_x / ts
	local cy = player.grid_y / ts
	...
	for _, n in ipairs(neighbours) do
		...
		if map[ny] and map[ny][nx] and map[ny][nx] ~= 1 then
			love.graphics.circle("fill", nx * ts + ts / 2, ny * ts + ts / 2, 5)
		end
	end
...
function drawDiceCutscene()
	...
	elseif e >= 3.0 then
		...
		love.graphics.printf(tostring(diceRoll.result), cx - size, cy - size + 10, size * 2, "center")
	else
...
function player_keypressed(key)
	...
	local ts    = Cfg.TILE_SIZE
	local moved = false
	local newX  = player.grid_x / ts
	local newY  = player.grid_y / ts

	if key == "up" and testMap(0, -1) then
		player.grid_y = player.grid_y - ts
		newY          = newY - 1
		moved         = true
	...
	elseif key == "left" and testMap(-1, 0) then
		player.grid_x = player.grid_x - ts
		newX          = newX - 1
		moved         = true
