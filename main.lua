...
require("lib.entities.dm")
require("lib.core.dm_hud")
...
function love.update(dt)
  ...
  elseif gameState == "dm_game" or gameState == "dm_lose" then
    ...
    if dm_hud_update  then dm_hud_update(dt)  end
...
function love.draw()
  ...
  elseif gameState == "dm_game" or gameState == "dm_lose" then
    ...
    -- Debug overlay drawn outside scissor so it's always visible
...
function love.keypressed(key)
  ...
  if key == "f5" then
    if gameState == "dm_game" then
      ...
      if gameState ~= "newGame" and gameState ~= "win" then
        worldSeed = os.time()
        grid_load(worldSeed)
        fog_load()
        player_load()
